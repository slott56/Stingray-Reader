...
def mock_workbook():
    mock_nav_name = Mock(
        name="Nav()",
        value=Mock(return_value=sentinel.VALUE)
    )
    mock_nav = Mock(
    )
...
def mock_builder():
    return Mock(
        return_value={"key": sentinel.KEY, "value": sentinel.VALUE}
    )
...
def test_main(tmp_path, mock_open_workbook, mock_persistent_processing, mock_process_workbook):
