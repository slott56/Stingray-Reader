...
We have two import constraints on the design,
described in `Essential Constraints`_.
First, the source value for each instance of a
schema is out of our control.
Second, we need to treat schema information as
immutable.

Additionally, there are a number of constraints
on the files and the way they are described by a
schema. These are physical format issues. We'll look at
`File and Schema Binding`_, `Loading a Schema`_, and `Some COBOL File Issues`_ that
will inform the rest of the design.

We'll dwell on some details of COBOL field
sizes and offsets in
`Formal Definitions for Non-Delimited Files`_.

Also, `JSON Schema Considerations`_ describes
how we can extend JSON Schema to support
the additional features required for schema
to describe non-delimited files.
...
1.  Each row of a spreadsheet or each record of a COBOL file
    is described by an opaque object outside the control
    of the **Stingray Reader** package. We will leverage
    external libraries to read spreadsheets, and we'll treat
    COBOL files as if they are similarly opaque.

2.  The schema information is essentially immutable.

The first constraint suggests that each Workbook format
requires a **Facade** class that make the workbook's details
...
In some cases, there is unique workbook-specific
processing that seems to be distinct from the
details of unpacking sheets, rows, and cell values
from the workbook.

The second constraint reflects a tradeoff decision.
...
Annotating (or decorating) the schema with size
and offset information makes it very difficult
to keep multiple rows in memory when they
have ``OCCURS DEPENDING ON`` constructs with distinct
values.

Keeping the size and offset information in separate
objects, permits a great deal more flexibility.
The cost associated with this flexibility is the
creation and destruction of numerous small :py:class:`Location`
instances.
